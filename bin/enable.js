...
var enable = function (argv, done) {
  var name = pkgSource = argv['_'][0];
  var sys = pkgSource = argv.sysinit;
  if(!name) return done(stdErrors.wrongUsage("nsi enable [service name]"))
  ...
  var execCmd = function (scMngr) {
    ...
    var child = (new Sapi()).enable(name, argv, function (err) {
      if (err) return done(err);
      console.log('service enabled: %s', name)
      done();
    })
