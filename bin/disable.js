...
var disable = function (argv, done) {
  var name = pkgSource = argv['_'][0];
  var sys = pkgSource = argv.sysinit;
  if(!name) return done(stdErrors.wrongUsage("nsi disable [service name]"))
  ...
  var execCmd = function (scMngr) {
    ...
    var child = (new Sapi()).disable(name, argv, function (err) {
      if (err) return done(err);
      console.log('service disabled: %s', name)
      done();
    })
    child && child.stderr && child.stderr.pipe(process.stderr); // upstart returns undefined
    child && child.stdout && child.stdout.pipe(process.stdout); // upstart returns undefined
  }
